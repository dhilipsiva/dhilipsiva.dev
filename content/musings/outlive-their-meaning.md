...
You can call `settle` on anything that knows how to compare itself to a previous version of itself. Nodes in [`nibli`](/nibli/)'s federated gossip do exactly this: they exchange knowledge until no opinion changes, and then they call that agreement. The optimism is not that agreement is guaranteed — it isn't. The optimism is that it is *worth iterating toward anyway*.
