...
export class Model {
     ...
     * @returns {string}
     */
    init_with_prompt(prompt, temp, top_p, repeat_penalty, max_tokens, seed) {
        let deferred3_0;
        let deferred3_1;
        try {
            const ptr0 = passStringToWasm0(prompt, wasm.__wbindgen_malloc, wasm.__wbindgen_realloc);
            const len0 = WASM_VECTOR_LEN;
            const ret = wasm.model_init_with_prompt(this.__wbg_ptr, ptr0, len0, temp, top_p, repeat_penalty, max_tokens, seed);
            var ptr2 = ret[0];
            var len2 = ret[1];
            if (ret[3]) {
                ptr2 = 0; len2 = 0;
     ...
     */
    constructor(gguf, tokenizer_json) {
...
async function __wbg_init(module_or_path) {
    ...
    if (module_or_path === undefined) {
        module_or_path = new URL('slm_wasm_bg.wasm', import.meta.url);
    }
    const imports = __wbg_get_imports();
